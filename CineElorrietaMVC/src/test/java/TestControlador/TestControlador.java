...
public class TestControlador {
	...
	Pelicula[] peliculas = new Pelicula[16];
	  ...
	  @Test public void testNavegarPanelGeneros() { 
	  spyControlador = spy(new Controlador(modeloMock, vistaMock)); doReturn(controladorGenerosMock)
	  .when(spyControlador) .makeControladorGeneros(any(Modelo.class),
	  any(Vista.class), any(Controlador.class));
	  
	  spyControlador.navegarPanelGeneros(); verify(spyControlador,
	  times(1)).navegarPanelGeneros(); }
	 // COMO SE HACE???
	/*
	 * doReturn(panelGeneroMock) .when(spyControlador)
	 * .makeControladorGeneros(any(Modelo.class), any(Vista.class),
	 * any(Controlador.class));
	 * 
	 * spyControlador.navegarPanelGeneros(); verify(vistaMock,
	 * times(1)).mostrarPanel(panelGeneroMock); }
	 * 
	 * /*spyControlador = spy(new Controlador(modeloMock, vistaMock));
	 * doReturn(controladorGenerosMock) .when(spyControlador)
	 * .makeControladorPanelGeneros(any(Modelo.class), any(Vista.class),
	 * any(Controlador.class));
	 * 
	 * 
